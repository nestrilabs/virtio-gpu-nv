...
impl Vm {
    ...
    // Return fd with a "shared ownership" handle (Arc)
    pub fn fd_shared(&self) -> std::sync::Arc<kvm_ioctls::VmFd> {
        self.fd.clone() // This clones the Arc (the pointer), NOT the VmFd itself.
    }
