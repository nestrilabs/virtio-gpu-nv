...
/*
 * Enable NVIDIA GPU sharing for this VM.
 * The VMM will open host /dev/nvidia* and proxy ioctls from the guest.
 * Multiple VMs can share the same GPU concurrently.
 * Arguments:
 *  "ctx_id"  VM context
 *  "gpu_ids"  Comma-separated GPU indices, e.g. "0" or "0,1". NULL = all.
 *  "caps"     Capability string: "compute,graphics,video,utility". NULL = all.
 * Returns:
 *  Zero on success or a negative errno on error.
 */
int32_t krun_enable_nvgpu(uint32_t ctx_id, const char *gpu_ids,
                          const char *caps);
